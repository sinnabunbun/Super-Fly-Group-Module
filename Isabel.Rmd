...
```
#Trying phylogram package
```{r}
install.packages("phylogram")
library(phylogram)
read.dendrogram(file = "matrix_from_app2_xl_file.nex")

#####This will convert a dataframe to a phylo object
install.packages("TreeSummarizedExperiment")
toTree(morph, column_order = NULL)
...
toTree(morph, column_order=NULL) #converts matrix to phylo object, but gives error of not allowed to have duplicated values in leaf column levona
duplicated(morph) #doesn't show any duplicates, can't find column levona?


morph2 <- morph[!duplicated(morph$levona),] 
toTree(morph2, column_order=NULL)
#removed the alleged duplicate, but now it says i need to reorder columns from high to low

#Made matrix in excel file a .csv, will try with that
morph.csv <- read.csv("Appendix 2 wilmott.csv")
toTree(morph.csv, column_order = NULL)

duplicated(morph.csv) #looking for duplicates in csv, there are 3
dedup_morph.csv <- unique(morph.csv, incomparables = FALSE) #removing duplicates
dedup_morph.csv

duplicated(dedup_morph.csv) #making sure the duplicates are gone, they are

toTree(dedup_morph.csv, column_order = NULL) #still says there is a duplicate in leaf column x.115
```
#Comparing
#Morphtree loading
```{r}
