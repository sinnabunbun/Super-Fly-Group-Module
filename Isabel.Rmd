...
comparePhylo(morph.tree, mol.tree, plot = FALSE, force.rooted = FALSE,
             use.edge.length = FALSE)
```
#We can also see that the morphological tree is rooted and the molecular tree is unrooted. We can either go back and adjust the molecular tree to root it, or we can adjust our code to force root both trees.
